...
* 재제출 허용 여부 검증
* **재제출 처리**:
  * 강사에 의해 `status`가 `resubmission_required`로 변경된 제출 건에 한해 재제출이 활성화된다.
  * 학습자가 다시 제출하면, 기존 `submissions` 레코드의 `content`, `link` 필드를 **갱신(UPDATE)**한다.
  * 레코드의 `status`는 다시 `submitted`로 변경하고, `is_late` 값은 현재 시각과 마감일을 비교하여 재계산한다.
* 텍스트/링크 유효성 검사
...
* 소유자만 접근 가능
* `draft → published` 전환 시 공개 (`published_at` 기록)
* `published → archived` 전환 시:
  - 신규 수강 차단
  - 모든 `published` 상태 과제를 `closed`로 자동 전환
  - 기존 수강생은 읽기 전용 접근만 가능 (제출 불가, 조회만 가능)
  - `archived_at` 타임스탬프 기록
* 코스 삭제 시 소프트 삭제 (`deleted_at` 기록, 물리적 삭제 금지)
* 카테고리/난이도는 `is_active = TRUE`인 항목만 선택 가능

**출력**

* 생성/수정/상태 전환 결과 반영
* 아카이브 시 관련 과제 상태 자동 변경
* 삭제 시 데이터 보존 (복구 가능)
...
* 소유 코스만 접근 가능
* 게시 시 학습자 노출 (`published_at` 기록)
* **과제 가중치 검증 (트랜잭션)**:
  - 과제 생성/수정 시 해당 코스의 모든 과제 `points_weight` 합계 계산
  - 합계가 1.0(100%)을 초과하면 트랜잭션 롤백 및 에러 반환
  - 이 검증은 데이터베이스 트랜잭션 내에서 수행
* 마감일 이후 자동 `closed` → **서버 스케줄러(Cron Job 등)를 통해 매일 자정(UTC 기준)에 마감일이 지난 `published` 상태의 과제를 일괄적으로 `closed` 상태로 변경하고 `closed_at` 기록**
* 제출물 테이블: 필터(미채점/지각/재제출요청)
* 과제 삭제 시 소프트 삭제 (`deleted_at` 기록, 제출물 이력 보존)
...
* 액션: 경고, 제출물 무효화, 계정 제한
* **메타데이터 비활성화 정책 (CRITICAL)**:
  - 물리적 `DELETE` 금지, `UPDATE SET is_active = FALSE` 사용
  - 사용 중인 메타데이터는 반드시 비활성화만 가능
  - 신규 생성 UI에는 `is_active = TRUE`인 항목만 표시
  - 기존 코스/과제는 비활성화된 메타데이터 참조 유지 (데이터 무결성)
* **사용자/코스 삭제 정책**:
  - 소프트 삭제 (`deleted_at` 기록) 사용
  - 강사 삭제 시 소유 코스 먼저 처리 필요 (`ON DELETE RESTRICT`)
  - 관련 데이터 보존으로 복구 가능성 확보
